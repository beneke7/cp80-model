...
.PHONY: all clean demo adapter_render adapter_check bench contact_probe pulse_probe pulse_sweep fit_pulse fit_hybrid lib check panel verify
all: $(B)/demo $(B)/adapter_render $(B)/adapter_check $(B)/analyze $(B)/bench $(B)/contact_probe $(B)/pulse_probe $(B)/pulse_sweep $(B)/stress $(B)/panel_demo $(B)/verify_stretch
$(B):            ; @mkdir -p $(B) out
$(B)/demo:    src/main.cpp     src/demo_score.hpp src/cp80_adapter.hpp src/cp80.hpp | $(B) ; $(CXX) $(CXXFLAGS) $(ARCH) $< -o $@
$(B)/adapter_render: tools/adapter_render.cpp src/demo_score.hpp src/cp80_adapter.hpp src/cp80.hpp | $(B) ; $(CXX) $(CXXFLAGS) $(ARCH) $< -o $@
$(B)/adapter_check: tools/adapter_check.cpp src/demo_score.hpp src/cp80_adapter.hpp src/cp80.hpp | $(B) ; $(CXX) $(CXXFLAGS) $(ARCH) $< -o $@
...
$(B)/stress:  tools/stress.cpp  src/cp80.hpp | $(B) ; $(CXX) $(CXXFLAGS) $(ARCH) $< -o $@
...
adapter_render: $(B)/adapter_render ; ./$(B)/adapter_render out/adapter-demo.wav
...
check: $(B)/stress $(B)/adapter_check ; ./$(B)/stress && ./$(B)/adapter_check  # stability + host-boundary gates
